        ...
        switch (tx.type) {
          ...
          case 'call':
            if (tx.logs.some(log => log.contractInfo)) {
              ...
              docDefinition.content.splice(5,0, {text: 'Transfers', fontSize: 15, alignment: 'center', bold: true, margin: [0, 30, 20, 20] })
              ...
              if (transfersList.length) {
                transfersList.forEach(transfer => {
                  ...
                  docDefinition.content.push({ text: makeUpperCase('Token transfer'), fontSize: 10, margin: [0, 20, 0, 10],  color: '#442A8E', border: { bottom: true, color: '#442A8E', width: 2 }});
